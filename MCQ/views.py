	...
	if request.user.is_authenticated():
		if request.user.get_username()=='admin':
			...
			if credentials.role=="student":
				...
				try:
					result=Result.objects.get(exam_name=exam_name,student=student)
					...
					try:
						...
						exam=Exam(exam_name=exam_name,question_paper=qp,answer_key=ak,exam_time=dt,exam_duration=120,course=c)
						exam.save()
						end_time=exam.exam_time+timedelta(minutes=exam.exam_duration)
